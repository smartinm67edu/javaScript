...
// Mostrar los Pokémon en la pantalla VS
function mostrarVS() {
    // Recuperar el Pokémon del jugador desde localStorage
    const playerPokemon = JSON.parse(localStorage.getItem("playerPokemon"));

    // Elegir un Pokémon oponente aleatorio
    const opponentPokemon = pokemons[Math.floor(Math.random() * pokemons.length)];
    const isShinyOpponent = Math.random() < 0.1;
    ...
    opponentContainer.innerHTML = `
        <h3>${opponentPokemon.name} ${isShinyOpponent ? "(Shiny)" : ""}</h3>
        <img src="${isShinyOpponent ? opponentPokemon.shinyImg : opponentPokemon.img}" alt="${opponentPokemon.name}">
    ...
    // Guardar oponente en localStorage para el combate
    localStorage.setItem("opponentPokemon", JSON.stringify({ ...opponentPokemon, isShiny: isShinyOpponent }));
}
